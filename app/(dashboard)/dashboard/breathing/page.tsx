  ...
  return (
    <>
      ...
      <main className="flex-1 px-8 sm:px-10 lg:px-12 py-8 overflow-y-auto bg-gradient-to-br from-slate-50/50 to-brand-surface/30">
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-text">Breathing Exercises</h1>
          <p className="text-text-muted">Master breathing techniques for labor and relaxation</p>
        </div>
          ...
          <CardContent className="p-8">
            <div className="flex items-center justify-between">
              <div className="text-white">
                <p className="text-sm opacity-80 mb-2">Today&apos;s Featured Exercise</p>
                <h2 className="text-2xl font-bold mb-2">{featuredExercise.name}</h2>
                <p className="opacity-80 mb-4">{featuredExercise.description}</p>
                <button
                  onClick={() => setSelectedExercise(featuredExercise)}
                  className="px-6 py-3 bg-white text-brand-mid rounded-lg font-medium hover:bg-brand-light transition-colors"
                >
                  Start Session
                </button>
              </div>
        ...
        <Card className="mb-8 bg-white/80 backdrop-blur-sm border-0 shadow-lg shadow-brand-dark/5">
          <CardHeader className="border-b-0">
            <h3 className="text-lg font-semibold text-text">All Exercises</h3>
            <p className="text-sm text-text-muted">Choose an exercise to practice</p>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {breathingExercises.map((exercise) => {
                ...
                return (
                  <div
                    key={exercise.name}
                    className="flex items-center justify-between p-4 rounded-xl border border-border/50 hover:border-brand-light hover:bg-brand-surface/30 transition-all"
                  >
                    <div className="flex items-center gap-4">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${isCompleted ? "bg-success/10 text-success" : "bg-brand-surface text-brand-mid"}`}>
                        {isCompleted ? (
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 text-sm text-text-muted">
                          ...
                          <span className="px-2 py-0.5 bg-brand-surface rounded-full text-xs">{exercise.type}</span>
                        </div>
                      </div>
                    </div>
                    <button
                      onClick={() => setSelectedExercise(exercise)}
                      className="px-4 py-2 border border-brand-mid text-brand-mid rounded-lg text-sm font-medium hover:bg-brand-mid hover:text-white transition-colors"
                    >
          ...
          <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg shadow-brand-dark/5">
            <CardHeader className="border-b-0">
              <div>
                ...
                <p className="text-sm text-text-muted">Your breathing practice history</p>
              </div>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                {sessions.slice(0, 10).map((session) => (
                  <div
                    key={session.id}
                    className="flex items-center justify-between p-4 rounded-xl bg-brand-surface/30"
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-medium text-text">{session.exercise_name}</p>
                    ...
                    <div className="text-right">
                      <p className="text-sm font-medium text-text">{formatDuration(session.duration_seconds)}</p>
                      <p className="text-xs text-text-muted capitalize">{session.exercise_type.replace("_", " ")}</p>
                    </div>
                  </div>
                ))}
              </div>
