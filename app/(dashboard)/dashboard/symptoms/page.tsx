    ...
    <>
      ...
      <main className="flex-1 px-4 sm:px-6 lg:px-12 py-6 sm:py-8 overflow-y-auto bg-gradient-to-br from-slate-50/50 to-brand-surface/30">
        <div className="mb-6 sm:mb-8">
          <h1 className="text-xl sm:text-2xl font-bold text-text">Symptom Tracker</h1>
          <p className="text-sm sm:text-base text-text-muted">Log and monitor your pregnancy symptoms</p>
        </div>

        {/* Quick Log */}
        <Card className="mb-6 sm:mb-8 bg-white/80 backdrop-blur-sm border-0 shadow-lg shadow-brand-dark/5">
          <CardHeader className="border-b-0 pb-2 px-4 sm:px-6">
            <h3 className="text-base sm:text-lg font-semibold text-text">Quick Log</h3>
            <p className="text-xs sm:text-sm text-text-muted">Tap to quickly log a symptom</p>
          </CardHeader>
          ...
          <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg shadow-brand-dark/5">
            <CardContent className="p-4 sm:p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs sm:text-sm text-text-muted">This Week</p>
                  <p className="text-2xl sm:text-3xl font-bold text-text">{stats.thisWeek}</p>
                  <p className="text-xs text-text-muted">symptoms logged</p>
                </div>
                <div className="p-2 sm:p-3 rounded-xl bg-brand-surface text-brand-mid">
            ...
            <CardContent className="p-4 sm:p-6">
              <div className="flex items-center justify-between">
                <div className="min-w-0 flex-1">
                  <p className="text-xs sm:text-sm text-text-muted">Most Common</p>
                ...
                <div className="p-2 sm:p-3 rounded-xl bg-warning/10 text-warning flex-shrink-0 ml-2">
                  <svg className="w-5 h-5 sm:w-6 sm:h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <CardContent className="p-4 sm:p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className={`text-lg sm:text-xl font-bold ${stats.hasMonitorStatus ? "text-warning" : "text-success"}`}>
                ...
                <div className={`p-2 sm:p-3 rounded-xl flex-shrink-0 ${stats.hasMonitorStatus ? "bg-warning/10 text-warning" : "bg-success/10 text-success"}`}>
                  <svg className="w-5 h-5 sm:w-6 sm:h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg shadow-brand-dark/5">
          <CardHeader className="border-b-0 px-4 sm:px-6">
            <div>
              ...
              <p className="text-xs sm:text-sm text-text-muted">Your symptom history</p>
            </div>
          </CardHeader>
          <CardContent className="px-4 sm:px-6">
            {symptoms.length === 0 ? (
              <div className="text-center py-8 sm:py-12">
                <div className="w-14 h-14 sm:w-16 sm:h-16 bg-brand-surface rounded-2xl flex items-center justify-center mx-auto mb-4">
                  <svg className="w-7 h-7 sm:w-8 sm:h-8 text-brand-mid" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" />
                  </svg>
                </div>
                <h3 className="text-base sm:text-lg font-semibold text-text mb-2">No symptoms logged yet</h3>
                <p className="text-sm text-text-muted">Use the quick log above to start tracking your symptoms</p>
              </div>
            ) : (
              <>
                  ...
                  {symptoms.map((symptom) => (
                    <div key={symptom.id} className="p-3 bg-brand-surface/30 rounded-xl">
                      <div className="flex items-start justify-between mb-2">
                        <div className="min-w-0 flex-1">
                          <span className="font-medium text-text text-sm">{symptom.name}</span>
                          {symptom.notes && (
                            <p className="text-xs text-text-muted mt-0.5 line-clamp-1">{symptom.notes}</p>
                          )}
                        </div>
                      ...
                      <div className="flex items-center justify-between text-xs text-text-muted">
                        <span>{formatDate(symptom.logged_at)} at {formatTime(symptom.logged_at)}</span>
